...
from free_slots import get_free_slots, format_free_slots_context, get_free_slots_for_room, format_free_slots_for_room_context
...
def build_context(parsed: dict, retrieval_results: list) -> str:
    ...
    # ── Free slots: always Python-computed ───────────────────────────────────
    ...
    if intent == "room_free_slots":
        ...
        if not day:
            return f"Could not determine which day you're asking about."
        result = get_free_slots_for_room(parsed["room"], day)
        return format_free_slots_for_room_context(result)

    # ── No retrieval results ──────────────────────────────────────────────────
    if not retrieval_results:
        return "No matching timetable entries found."

    # ── Faculty schedule: group by day for readability ────────────────────────
    if intent == "faculty_schedule":
        ...
        # Collect unique subjects — normalize Tut-MA-I(B3a/b/c) variants
        unique_subjects = []
        seen = set()
        for r in retrieval_results:
            subj = r.get("subject", "")
            # Normalize tutorial variants to their base name
        ...
        if day:
            ...
            for r in retrieval_results:
                lines.append(
                    ...
                    f"teaches {r.get('subject')} in room {r.get('room')} "
                    f"for {r.get('sem')}"
                )
        elif slot:
            ...
            # Full schedule — always list unique subjects first so LLM sees them clearly
    ...
    if intent == "room_availability":
        ...
        for r in retrieval_results:
            lines.append(
                ...
                f"{r.get('subject')} taught by {r.get('faculty')} "
                f"for {r.get('sem')}"
            )
        return "\n".join(lines)
...
def generate_answer(parsed: dict, retrieval_results: list, user_query: str) -> str:
    context = build_context(parsed, retrieval_results)
    prompt  = ANSWER_PROMPT.format(context=context, question=user_query)
